...
#include "opengl/4ed_opengl_funcs.h"
  ...
  if (register_success){
    ...
    HWND wnd = CreateWindowExW(0, 
                               ...
                               this_instance, 
                               0);
    /* NOTE(lake): The two lines below will make the background FULLY transparent but we don't want that.
     *             We want to achieve a windows acrylic style of background.
     */
    //SetWindowLong(wnd, GWL_EXSTYLE, GetWindowLong(wnd, GWL_EXSTYLE) | WS_EX_LAYERED);
    //SetLayeredWindowAttributes(wnd, RGB(18,18,18), 204, LWA_ALPHA | LWA_COLORKEY);
